...
optrega(void *val, opt_TYPE otype, char name, char *longname, char *descript)
{
    ...
    optlist[opt_nreg-1].longname      = longname ? _strdup(longname) : NULL;
    optlist[opt_nreg-1].mode          = mode;
    optlist[opt_nreg-1].descript      = descript ? _strdup(descript) : NULL;
...
opt_parse_longdelim(ARGVECTOR *ag)
{
    ...
    sc = _strdup(ag_s(ag));      /* long option name is a string */
    scptr = sc;
    while( !ag_eow(ag) ) {
        c=ag_c_advance(ag);
        if (c == '=') {
...
opt_action(int i, ARGVECTOR *ag)
{
    ...
    case OPT_STRING:
        ...
        OPT_SET_ITH_VALUE(char *,i,_strdup(s));
